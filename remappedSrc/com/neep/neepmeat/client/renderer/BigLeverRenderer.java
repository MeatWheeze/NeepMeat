...
public class BigLeverRenderer<T extends BigLeverBlockEntity> implements BlockEntityRenderer<T>
{
    ...
    public void render(T be, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, int overlay)
    {
        ...
        switch (face)
        {
            case FLOOR -> matrices.multiply(Vec3f.POSITIVE_Y.rotation((float) (Math.PI)));
            case WALL -> matrices.multiply(Vec3f.POSITIVE_X.rotation((float) (Math.PI / 2)));
            case CEILING -> matrices.multiply(Vec3f.POSITIVE_X.rotation((float) (Math.PI)));
        }
