...
public class EncasedDataCableBlockEntity extends SyncableBlockEntity implements EncasedBlockEntity
{
    private BlockState camoState = Blocks.AIR.getDefaultState();
    private VoxelShape cachedShape = null;

    public EncasedDataCableBlockEntity(BlockEntityType<?> type, BlockPos pos, BlockState state)
    {
    ...
    public VoxelShape getCamoShape()
    {
        if (cachedShape == null)
        {
            ...
            {
                if (getCachedState().getBlock() instanceof EncasedDataCableBlock block)
                {
